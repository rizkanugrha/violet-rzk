...
if (!existsSync('./src/assets/temp')) {
	mkdirSync('./src/assets/temp', { recursive: true })
}
...
export const download = (url, extension, optionsOverride = {}) => {
	return new Promise(async (resolve, reject) => {
		try {
			let stream = await getHttpStream(url, optionsOverride)
			const buffer = await toBuffer(stream)
			const type = await fileTypeFromBuffer(buffer)
			let filepath = `./src/assets/temp/${new Date().getTime()}.${extension || type.ext}`
...
export const upload = {
	async pomf(media) {
		try {
			media = Buffer.isBuffer(media) ? media : existsSync(media) ? readFileSync(media) : media
	...
	async uploadImage(buffer) {
		try {
			buffer = Buffer.isBuffer(buffer) ? buffer : existsSync(buffer) ? readFileSync(buffer) : buffer
			const { ext } = await fileTypeFromBuffer(buffer);
			const form = new FormData()
			form.append('files[]', buffer, 'tmp.' + ext);
			const res = await axios.post('https://cdn.nekohime.xyz/upload', form, {
				headers: form.getHeaders()
			})
			const img = await res.data.files[0]
			if (img.error) throw img.error
			return img.url

		} catch (e) {
			console.error(e + `nekonhime`)
		}
	},
