    ...
    execute: async (m, client, { body, prefix, args, arg, cmd }) => {
        try {
            ...
            if (m.quoted) {
                ...
                await client.groupParticipantsUpdate(m.from, [_user], 'demote')
                ...
                m.reply(`Demoting *${_participants.length}* group members to be Group Admin within delay 3 seconds to prevent banned`)
                for (let usr of _participants) {
                    if (usr.endsWith('@s.whatsapp.net') && !(_participants.find(v => areJidsSameUser(v.id, usr)))) {

                        await delay(3000)
                        await client.groupParticipantsUpdate(m.from, [usr], 'demote')
                    }
                }
